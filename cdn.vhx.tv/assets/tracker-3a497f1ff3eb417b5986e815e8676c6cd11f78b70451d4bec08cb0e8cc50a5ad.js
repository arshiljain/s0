...
var webSdkAllowed = function () {
  var configElement = document.getElementById('tracking-config'),
    config,
    purpose,
    consent;
  ...
  purpose = config.sdk_purposes && config.sdk_purposes.vimeo_platform_events;
  if (!purpose) {
    return null;
  }
  ...
  consent = getCookie('gcp');
  if (!consent) {
    return config.sdk_allowed && config.sdk_allowed.vimeo_platform_events;
  }
