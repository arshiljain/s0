if (typeof isSellUpjs__running === 'undefined') {
  ...
  // Merchant-editable style settings (colors/sizes) get concatenated into
  // style="..." attributes inside HTML strings elsewhere in this file, which
  // are then parsed as markup (jQuery $(...), .html(), addContentBox, etc.).
  // Without validation, a value like `red;"><script>...` breaks out of the
  // attribute and injects arbitrary markup into every storefront visitor's
  // page. These allow-list any value used in that position instead of
  // passing merchant input straight into the attribute.
  
  function ltSanitizeColor(v) {
    if (v === null || v === undefined) return '';
    var s = String(v).trim();
    return /^(#[0-9a-fA-F]{3,8}|rgba?\([0-9.,%\s]+\)|hsla?\([0-9.,%\s]+\)|[a-zA-Z]+)$/.test(s) ? s : '';
  }
  function ltSanitizeSize(v) {
    if (v === null || v === undefined) return '';
    var s = String(v).trim();
    return /^[0-9]+(\.[0-9]+)?$/.test(s) ? s : '';
  }
  function ltSanitizeFontFamily(v) {
    if (v === null || v === undefined) return '';
    var s = String(v).trim();
    return /^[a-zA-Z0-9 \-]+$/.test(s) ? s : '';
  }
  function ltEscapeHtml(v) {
    if (v === null || v === undefined) return '';
    return String(v)
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
            ...
            if (cart_type !== 'popup') {
              ...
              if ($cart_drawer_selector.length) {
                ...
                if(!script_featherlight_has_created){
                  var launchtip_script = document.createElement('script');
                  launchtip_script.type = 'text/javascript';
                  launchtip_script.integrity = 'sha384-w2axFeF9Tj4TdpJI51s6IO16THIEjqTxuxTeDLfLY45ZpuoVkl/s9zF0N3mJg0oX';
                  launchtip_script.crossOrigin = 'anonymous';
                  launchtip_script.src = "https://cdn.jsdelivr.net/npm/featherlight@1.7.14/release/featherlight.min.js";
                  document.head.appendChild(launchtip_script);
                    ...
                    success: function(data) {
                      if (data['items'].length != 0) {
                        ...
                        if (items_ids != undefined) {
                            var url = "/apps/launchtip/output/cart_view.json?shop=" + shop_name + "&items=" + encodeURIComponent(JSON.stringify(items_ids)) + "&shop_locale=" + Shopify.locale;
                          ...
                          success: function(data) {
                              ...
                              if(data["cart_upsells_found_or_not"] == true || data["upcart_heading"]["default_cart_use"] != 'auto'){
                                ...
                                if(sellup_ids.length != 0){
                                  $.each( sellup_ids, function(index, value){
                                    if(value != ""){
                                      var url_ind_pr = '/apps/launchtip/output/individual_detail.json?sellup_id='+value;
                                      $.ajax({
                                        crossOrigin: true,
                                        url: url_ind_pr,
                                        success: function(data) {
                            ...
                            function main_success() {
                              ...
                              if(data["settings"]["up_price_font_size"] != ''){
                                  pricestylestoapply += 'font-size:'+ltSanitizeSize(data["settings"]["up_price_font_size"])+'px;'
                              }
                              if(data["settings"]["up_price_font_color"] != ''){
                                  pricestyletoapply += 'color:'+ltSanitizeColor(data["settings"]["up_price_font_color"])+' !important;';
                                  pricestyletoapply += 'font-size:'+ltSanitizeSize(data["settings"]["up_price_font_size"])+'px;'
                              }
                              if(data["settings"]["up_comp_price_font_color"] != ''){
                                  comppricestyletoapply += 'color:'+ltSanitizeColor(data["settings"]["up_comp_price_font_color"])+';';
                                  comppricestyletoapply += 'font-size:'+ltSanitizeSize(data["settings"]["up_price_font_size"])+'px;'
                              ...
                              if(status == true){
                                  var producthtml = function(productdetail,count,cart_upsell_no,selected_id,sellup_id,ind_product_id){
                                      ...
                                      $.ajax({
                                          ...
                                          error: function() {
                                              var url1 = '/apps/launchtip' + '/output/upsell_offer_product_data.json?shop=' + shop_name + '&product_id=' + ind_product_id;
                                      ...
                                      if(prod == null){
                                          ...
                                          var addcart_text = ltEscapeHtml(data["settings"]["cart_upsell_addcart"]);
                                          ...
                                          var popup_heading_text_value = ltEscapeHtml(data["settings"]["popup_heading_text"]);
                                          var btnstyletoapply = '';
                                          var btnstyletoapply_drawer = '';
                                          if(data["settings"]["up_button_font_size"] != ''){
                                            btnstyletoapply += 'font-size:'+ltSanitizeSize(data["settings"]["up_button_font_size"])+'px;';
                                          }

                                          if(data.settings?.up_button_font_size){
                                            btnstyletoapply_drawer += 'font-size:'+ltSanitizeSize(data["settings"]["up_button_font_size"])+'px;';
                                          }else{
                                          ...
                                          if(data.settings?.up_button_font_color){
                                            ...
                                            btnstyletoapply_drawer += 'color:' + ltSanitizeColor(data["settings"]["up_button_font_color"]) + ';';
                                          ...
                                          if(data.settings?.up_button_bg_color){
                                            ...
                                            btnstyletoapply_drawer += 'background-color:' + ltSanitizeColor(data["settings"]["up_button_bg_color"]) + ';';
                                            ...
                                            return '<div id="product-' + prod["handle"] + '" class="item productbox launchtip_cart_upsell_product_in_drawer upsellproductcount' + count + '" style="border: 1px solid var(--color-border, #dadce0); padding: 10px; display: inline-flex; align-items: center;  width: 340px; max-width: 100%; position: relative; background: var(--color-product-card-bg, #fff); border-radius:10px;">' + 
                                              ...
                                              '<div class="details launchtip_cart_drawer_product_title" style="font-size:.75rem;min-width: 215px; cursor: pointer;">' + 
                                                '<a href="' + window.Shopify.routes.root + 'products/' + prod["handle"] + '" style=" display: block; font-size: calc(var(--font-product-title-scale, 1) * 0.875rem); line-height: calc(var(--font-product-title-line-height-scale, 1) * 1.4); font-weight: var(--font-body-medium-weight, 500); margin-bottom: 3px; text-decoration: none; font-weight: 500; font-size: ' + 
                                                  (data["settings"]["title_font_size"] ? ltSanitizeSize(data["settings"]["title_font_size"]) + 'px' : '14px') + 
                                                  '; color:' + ltSanitizeColor(data["settings"]["up_title_font_color"]) + '">' + 
                                                ...
                                                '<div class="launchtip_upsell_lightbox launchtiplightbox upsell_variants" id="fl' + count + cart_upsell_no + '">' + 
                                                  '<div class="sellup_popup_heading">' + popup_heading_text_value + '</div>' + 
                                                  '<div class="lbimg">' + 
                              ...
                              var upsellheading = ltEscapeHtml(data["settings"]["cart_title"])
                  ...
                  var configuration = ({
                    ...
                    beforeOpen: function(event){
                      var variant_popup_bg_color = (ltSanitizeColor(setting['popup_bg_color']) == '' || ltSanitizeColor(setting['popup_bg_color']) == null) ? '#fff' : ltSanitizeColor(setting['popup_bg_color']);
                      $('.launchtip_upsell_lightbox .sellup_popup_heading').css("color", ltSanitizeColor(setting['popup_font_color']));
                      $('.featherlight-close').attr("style", "background-color: transparent !important; color: "+ltSanitizeColor(setting['popup_button_color'])+";");
                      $('.launchtip_upsell_lightbox select, .launchtip_upsell_lightbox input').attr("style", "background-color: "+ltSanitizeColor(setting['popup_bg_color'])+" !important; color: "+ltSanitizeColor(setting['popup_font_color'])+";");
                      $('.featherlight-content').attr("style", "background-color: "+variant_popup_bg_color+" !important; border-color: "+ltSanitizeColor(setting['popup_border_color'])+" !important; color: "+ltSanitizeColor(setting['popup_font_color'])+";");
                  ...
                  function get_selectect_variant_lightbox(variant_id,form){
                    ...
                    if (selected_variant.available == false) {
                        $(form).find('button').find('span').text(setting["sold_out_text"] || "Sold Out")
                  ...
                  function handleAddToCartClick(e){
                    ...
                    $(thiselement).find('span').text(setting["button_text_on_ajax"] || "Adding...");
                      ...
                      success: function(line_item) { 
                        ...
                        if ((typeof callback) === 'function') {
                          ...
                          // ----------------end---------------
                           ...
                           $(thiselement).find('span').text(setting["button_text_on_ajax"] || "Added");
                            setTimeout(function() {
  ...
  if(template_name == "product" || thank_you_page == true || order_preview_page == true){
     ...
     // duplicate offer products
      ...
      jQuery(document).ready(function($) {
        ...
        function addGoogleFont(FontName) {
          var font_link = document.createElement("font_link");
          font_link.rel = "stylesheet";
          font_link.type = "text/css";
          font_link.href = "https://fonts.googleapis.com/css?family=" + encodeURIComponent(FontName);
        ...
        if (thank_you_page == true || order_preview_page == true) {
          var url = '/apps/launchtip' + '/output/post_upsell.json?shop=' + shop_name;
            ...
            success: function(data) {
              if (data["upsell_data"]["upseller_status"] == true) {
                ...
                if (setting.up_price_font_size != '' && setting.up_price_font_size != null) {
                  pricestylestoapply += 'font-size:' + ltSanitizeSize(setting.up_price_font_size) + 'px;'
                }
                if (setting.up_price_font_color != '' && setting.up_price_font_color != null) {
                  pricestyletoapply += 'color:' + ltSanitizeColor(setting.up_price_font_color) + ';';
                }
                if (setting.up_comp_price_font_color != '' && setting.up_comp_price_font_color != null) {
                  comppricestyletoapply += 'color:' + ltSanitizeColor(setting.up_comp_price_font_color) + ';';
                }
                ...
                if (data["upsell_data"]["upseller_status"] == true) {
                  if (data["upsell_data"]["post_upsell_title"].includes("{{ countdown }}") == true) {
                    ...
                    var post_upsell_title = ltEscapeHtml(data["upsell_data"]["post_upsell_title"].replace('{{ countdown }}', ''));

                    var countdown_span = '<span class="upsell_countdown"></span>';
                  } else {
                    var post_upsell_title = ltEscapeHtml(data["upsell_data"]["post_upsell_title"]);
                    var countdown_span = '';
                  }

                  if (data["upsell_data"]["upseller_lead_product_mode"] == "product") {

                    var title = ltEscapeHtml(data["upsell_data"]["upseller_lead_prod_title"])
                    var image = data["upsell_data"]["upseller_lead_prod_img_url"]
                    var handle = ltEscapeHtml("/products/" + data["upsell_data"]["upseller_lead_prod_handle"])

                  } else if (data["upsell_data"]["upseller_lead_product_mode"] == "collection") {
                    var title = ltEscapeHtml(data["upsell_data"]["upseller_lead_collect_title"])
                    var image = data["upsell_data"]["upseller_lead_collect_img"]
                    var handle = ltEscapeHtml("/collections/" + data["upsell_data"]["upseller_lead_collect_handle"])
                    var final_price_v = "";
                  }

                  var btnstyletoapply = '';
                  if (setting.up_button_font_size != '') {
                    btnstyletoapply += 'font-size:' + ltSanitizeSize(setting.up_button_font_size) + 'px;'
                  }
                  if (setting.up_button_bg_color != '') {
                    btnstyletoapply += 'background-color:' + ltSanitizeColor(setting.up_button_bg_color) + ';';
                  }
                  if (setting.up_button_font_color != '') {
                    btnstyletoapply += 'color:' + ltSanitizeColor(setting.up_button_font_color) + ';';
                  }
                  if (image != '') {
        ...
        var url = '/apps/launchtip'+'/output/sellup_shop_data.json?shop='+shop_name+'&id='+product_id+'&locale='+shop_locale;
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            compact_mode_offers_data["offers_expend_text"] = setting["offers_expend_text"] == '' ? 'View More' : ltEscapeHtml(setting["offers_expend_text"])
            compact_mode_offers_data["offers_shrink_text"] = setting["offers_shrink_text"] == '' ? 'View Less' : ltEscapeHtml(setting["offers_shrink_text"])
           if (data['lead_pro_data']) {
            ...
            if (template == "product") {
              ...
              var styleElement_checkbox = document.createElement('style');
                styleElement_checkbox.type = 'text/css';

                    styleElement_checkbox.textContent =
                  '#launchtip_upsell_select_wrapper input[type=checkbox]:checked + label:before {' +
                  btnstyletoapply +
                  '}';

                document.head.appendChild(styleElement_checkbox);

              // if(setting.up_style == "custom"){
                 var custom_css_lt = document.createElement('style');
                  custom_css_lt.textContent = setting.up_custom_css;
                  document.head.appendChild(custom_css_lt);
              // }
              ...
              var url = '/apps/launchtip'+'/output/view?shop='+ shop_name+'&product_id='+product_id+'&upsell_type=onpage';
                ...
                if (sorted_upsellers_action.length > 0) {
                  ...
                  if (sortedactionoffers.length > 0) {
                   upsell_action_data = removeDuplicateOfferProds(sortedactionoffers);
                      ...
                      if (upsell_action_data.length && setting?.translations) {
                        upsell_action_data.forEach(item => {
                          item.action_popup_title =
                            setting.translations[`action_popup_title_${item.id}`]?.[Shopify.locale] ||
                            item.action_popup_title;

                          item.action_popup_description =
                            setting.translations[`action_popup_description_${item.id}`]?.[Shopify.locale] ||
                            item.action_popup_description;

                          item.action_upsell_continue =
                            setting.translations[`action_upsell_continue_${item.id}`]?.[Shopify.locale] ||
                            item.action_upsell_continue;
                        });
                      }
              ...
              function get_selectect_variant_lightbox(variant_id, form, type) {
                ...
                if ((selected_variant_firstvariantcprice) && (selected_variant_firstvariantcprice != '') && (selected_variant_firstvariantcprice > selected_variant_firstvariantprice)) {
                  ...
                  if (setting.up_comp_price_text != '') {
                    comp_text = ltEscapeHtml(setting.up_comp_price_text);
                  }
                  ...
                  var comppricestyletoapply = '';
                  if (setting.up_price_font_size != '' && setting.up_price_font_size != null) {
                    pricestyletoapply += 'font-size:' + ltSanitizeSize(setting.up_price_font_size) + 'px;'
                  }
                  if (setting.up_price_font_color != '' && setting.up_price_font_color != null) {
                    pricestyletoapply += 'color:' + ltSanitizeColor(setting.up_price_font_color) + ';';
                  }
                  if (setting.up_comp_price_font_color != '' && setting.up_comp_price_font_color != null) {
                    comppricestyletoapply += 'color:' + ltSanitizeColor(setting.up_comp_price_font_color) + ';';
                    comppricestyletoapply += 'font-size:' + ltSanitizeSize(setting.up_price_font_size) + 'px;'
                  }
                ...
                if (setting.up_price_text != '' && cp_html != '') {
                  price_text = ltEscapeHtml(setting.up_price_text);
                }
              ...
              var producthtml = function(ind_product_id, sellup_id, handle, count, upsell_setting, selected_id, upsell_col_id,action_type) {
                ...
                if (prod == null) {
                  ...
                  if (setting.title_font_size != '' && setting.title_font_size != null) {
                    titlestyletoapply += 'font-size:' + ltSanitizeSize(setting.title_font_size) + 'px;'
                  }
                  if (setting.up_title_font_color != '' && setting.up_title_font_color != null) {
                    titlestyletoapply += 'color:' + ltSanitizeColor(setting.up_title_font_color) + ';';
                  }
                  ...
                  if (setting.up_comp_price_font_size != '' && setting.up_comp_price_font_size != null) {
                    comppricestyletoapply += 'font-size:' + ltSanitizeSize(setting.up_comp_price_font_size) + 'px;'
                  }
                  if (setting.up_comp_price_font_color != '' && setting.up_comp_price_font_color != null) {
                    comppricestyletoapply += 'color:' + ltSanitizeColor(setting.up_comp_price_font_color) + ';';
                  }
                  ...
                  if (upsell_setting.action_offer_type == 'upsell') {
                    if (setting.action_atc_button_text != "" && setting.action_atc_button_text !== null) {
                      btn_text = ltEscapeHtml(setting.action_atc_button_text);
                    } else {
                      btn_text = "Select";
                    }
                  } else if (goto_pro_or_not == true){
                    btn_text = setting.redirect_to_product_text != '' ? ltEscapeHtml(setting.redirect_to_product_text) : "Customize";
                  } else {
                    if (setting.up_button_text != "" && setting.up_button_text !== null) {
                      btn_text = ltEscapeHtml(setting.up_button_text);
                    } else {
                      btn_text = "Add";
                    }
                  }
                  ...
                  if (setting.up_button_bg_color != '') {
                    popup_btnstyletoapply += 'background-color:' + ltSanitizeColor(setting.up_button_bg_color) + ';';
                  }
                  if (setting.up_button_font_color != '') {
                    popup_btnstyletoapply += 'color:' + ltSanitizeColor(setting.up_button_font_color) + ';';
                  }
                  if (setting.up_button_font_size != '') {
                    popup_btnstyletoapply += 'font-size:' + ltSanitizeSize(setting.up_button_font_size) + 'px;';
                  }
                  ...
                  if (setting.popup_heading_text != '') {
                    popup_heading_text_value = ltEscapeHtml(setting.popup_heading_text);
                  }

                  var productboxstyletoapply = '';
                  var applyborderstyle = false;
                  if (setting.up_border_top_size != '') {
                    productboxstyletoapply += 'border-top-width:' + ltSanitizeSize(setting.up_border_top_size) + 'px;'
                    applyborderstyle = true;
                  }
                  if (setting.up_border_left_size != '') {
                    productboxstyletoapply += 'border-left-width:' + ltSanitizeSize(setting.up_border_left_size) + 'px;'
                    applyborderstyle = true;
                  }
                  if (setting.up_border_right_size != '') {
                    productboxstyletoapply += 'border-right-width:' + ltSanitizeSize(setting.up_border_right_size) + 'px;'
                    applyborderstyle = true;
                  }
                  if (setting.up_border_bottom_size != '') {
                    productboxstyletoapply += 'border-bottom-width:' + ltSanitizeSize(setting.up_border_bottom_size) + 'px;'
                    applyborderstyle = true;
                  }
                  if (setting.up_border_color != '') {
                    productboxstyletoapply += 'border-color:' + ltSanitizeColor(setting.up_border_color) + ';';
                    applyborderstyle = true;
                  }
                  if (setting.up_border_padding != '') {
                    productboxstyletoapply += 'padding:' + ltSanitizeSize(setting.up_border_padding) + 'px;';
                    applyborderstyle = true;
                  }
                  ...
                  if (setting.up_title_price_select_font != '') {
                    ...
                    font = font.split(':');
                    title_font_css += 'font-family:' + ltSanitizeFontFamily(font[0]);
                  }
                  ...
                  if (vcount == 1 && selected_variant.available == false) {
                    ...
                    var quickview_id = 'qv' + count + upsell_col_id + popup_ct_val;
                    var title_anchor_open = (upsell_setting.open_in_modal == true)
                      ? '<a class="detail_title openbox" href="#' + quickview_id + '">'
                      : '<a class="detail_title" href="' + window.Shopify.routes.root + 'products/' + prod["handle"] + '">';
                    var quickview_thumbs_html = '';
                    if (prod["images"] && prod["images"].length > 1) {
                      quickview_thumbs_html = '<div class="launchtip_quickview_thumbs">' + prod["images"].map(function(img_src) {
                        return '<img class="launchtip_quickview_thumb' + (img_src == selected_variant_img ? ' active' : '') + '" src="' + img_src + '" alt="">';
                      }).join('') + '</div>';
                    }

                    var quickview_product_url = window.Shopify.routes.root + 'products/' + prod["handle"];
                    var quickview_image_block = (!selected_variant_img) ? '' : '\
                      <div class="lbimg">\
                      <a href="' + quickview_product_url + '"><img style="max-width: 100%;height: auto;width:100%;display: block;" class="lightboximage" src="' + selected_variant_img + '" alt="' + prod["title"].replace(/"/g, '').replace(/'/g, '') + '" /></a>\
                      ' + quickview_thumbs_html + '\
                      </div>';
                    var quickview_has_variant_form = (selected_id == '' && vcount > 1);
                    var quickview_atc_html = quickview_has_variant_form ? popupform : link_html;
                    var quickview_qty_stepper_html = '<div class="launchtip_quickview_qty_wrapper">\
                      <button type="button" class="launchtip_quickview_qty_btn launchtip_quickview_qty_minus">−</button>\
                      <input type="number" name="quantity" min="1" value="1" title="Quantity" class="launchtip_quickview_qty_input">\
                      <button type="button" class="launchtip_quickview_qty_btn launchtip_quickview_qty_plus">+</button>\
                    </div>';
                    quickview_atc_html = quickview_atc_html
                      .replace('<input type="hidden" name="quantity" value="1" tabindex="-1">', quickview_qty_stepper_html)
                      .replace('<div class="launchtip_select_option_wrapper"><label>Quantity</label><input type="number" name="quantity" min="1" value="1" title="Quantity" style="width: 50%;"></div>', quickview_qty_stepper_html)
                      .replace('<input type="number" name="quantity" min="1" value="1" title="Quantity">', quickview_qty_stepper_html);
                    var quickview_price_html = quickview_has_variant_form ? '' : ('<div class="launchtip_quickview_price">' + price_get(count) + '</div>');
                    var launchtip_quickview_modal = (upsell_setting.open_in_modal != true) ? '' : '\
                      <div class="launchtip_upsell_lightbox launchtip_quickview_lightbox' + (selected_variant_img ? '' : ' launchtip_quickview_noimage') + '" id="' + quickview_id + '">\
                      ' + quickview_image_block + '\
                      <div class="launchtip_quickview_details">\
                      <h3><a href="' + quickview_product_url + '">' + title_get(count) + '</a></h3>\
                      ' + quickview_price_html + '\
                      <div class="launchtip_quickview_atc">' + quickview_atc_html + '</div>\
                      <div class="launchtip_quickview_description">' + (prod["description"] || '') + '</div>\
                      </div>\
                      </div>';
                ...
                if (prod == null) {
                  ...
                  if (selected_id != '') {
                    ...
                    if (vcount == 1) {
                      ...
                      var availableVariant = prod["variants"].find(
                        variant => variant.available === true
                      );
                      var selected_variant_img =
                        availableVariant?.featured_image?.src || prod["featured_image"];
                  }
                  if (typeof selected_variant === 'undefined') {
                    var selected_variant = prod["variants"][0]
                  }
                      ...
                      if (setting.up_button_bg_color && setting.up_button_bg_color.trim() !== '') {
                          ...
                          style.innerHTML = `
                              input:checked + .sellup_toggle_slider {
                                  background-color: ${ltSanitizeColor(setting.up_button_bg_color)} !important;
                              }
                          `;
                    ...
                    if (selected_variant_img == null) {
                      product_image_container = "";
                          ...
                          {return '';}
                           ...
                           var quickview_id = 'qv' + count + upsell_col_id + 'c';
                            var title_anchor_open = (upsell_setting.open_in_modal == true)
                              ? '<a class="openbox" href="#' + quickview_id + '"'
                              : '<a href=" '+window.Shopify.routes.root+'products/'+prod["handle"]+'"';
                            var quickview_thumbs_html = '';
                            if (prod["images"] && prod["images"].length > 1) {
                              quickview_thumbs_html = '<div class="launchtip_quickview_thumbs">' + prod["images"].map(function(img_src) {
                            ...
                            var quickview_btnstyletoapply = '';
                            if (setting.up_button_bg_color != '') { quickview_btnstyletoapply += 'background-color:' + ltSanitizeColor(setting.up_button_bg_color) + ';'; }
                            if (setting.up_button_font_color != '') { quickview_btnstyletoapply += 'color:' + ltSanitizeColor(setting.up_button_font_color) + ';'; }
                            if (setting.up_button_font_size != '') { quickview_btnstyletoapply += 'font-size:' + ltSanitizeSize(setting.up_button_font_size) + 'px;'; }
                            var quickview_atc_btn_text = (setting.up_button_text != '') ? ltEscapeHtml(setting.up_button_text) : 'Add to Cart';
                            var quickview_atc_button_html = '<button type="submit" name="add" ind_product_id="' + ind_product_id + '" sell-id="' + sellup_id + '" upsell-type="' + upsell_setting.upsell_type + '" class="btn add-to-cart launchtip_add_to_cart" style="' + quickview_btnstyletoapply + 'border: none;cursor: pointer;width:100%;"><span class="add-to-cart__text" data-text="' + quickview_atc_btn_text + '">' + quickview_atc_btn_text + '</span></button>';
                            var quickview_qty_html = '<div class="launchtip_quickview_qty_wrapper">\
                              <button type="button" class="launchtip_quickview_qty_btn launchtip_quickview_qty_minus">−</button>\
                              <input type="number" name="quantity" min="1" value="1" title="Quantity" class="launchtip_quickview_qty_input">\
                              <button type="button" class="launchtip_quickview_qty_btn launchtip_quickview_qty_plus">+</button>\
                            </div>';
                            var quickview_has_variant_form = (selected_id == '' && vcount > 1);
                            if (quickview_has_variant_form) {
                              var quickview_allprodoption = '';
                              $.each(prod["options"], function(key, value) {
                                var voptionlist = '';
                                var select_lable = "<label>" + value.name + "</label>";
                                $.each(value["values"], function(key1, val) {
                                ...
                                quickview_allprodoption += '<div class="launchtip_select_option_wrapper">' + select_lable + '<select class="launchtip_select_option_list">' + voptionlist + '</select></div>';
                              });
                              var quickview_atc_html = '<form action="#" method="post" class="LaunchTipAddToCartForm launchtipsellupform" enctype="multipart/form-data">\
                                <div class="provariantdata" style="display:none;">' + JSON.stringify(prod) + '</div>\
                                <div class="popupmsg" style="height:0;"></div>\
                                <div class="popuppricediv quickview_price">' + price_get(count) + '</div>\
                                <select name="id" class="launchtip_select_wrapper" style="display:none;">' + voption1 + '</select>\
                                <div class="launchtip_select_options_div">' + quickview_allprodoption + quickview_qty_html + '</div><input type="hidden" name="properties[_sellup_id]" value="' + sellup_id + '">' + selling_plan_field + parent_attribution_content + quickview_atc_button_html + '</form>';
                            } else {
                              var quickview_atc_html = '<form action="#" method="post" class="LaunchTipAddToCartForm" enctype="multipart/form-data">\
                                <input type="hidden" name="id" value="' + selected_id + '">' + quickview_qty_html + '<input type="hidden" name="properties[_sellup_id]" value="' + sellup_id + '">' + selling_plan_field + parent_attribution_content + quickview_atc_button_html + '</form>';
                            }
                              var launchtip_quickview_modal = (upsell_setting.open_in_modal != true) ? '' : '\
                              <div class="upline">\
                              <div class="launchtip_upsell_lightbox launchtip_quickview_lightbox' + (selected_variant_img ? '' : ' launchtip_quickview_noimage') + '" id="' + quickview_id + '">\
                              ...
                              <div class="launchtip_quickview_details">\
                              <h3><a href="' + quickview_product_url + '">' + title_get(count) + '</a></h3>\
                              ' + (quickview_has_variant_form ? '' : ('<div class="launchtip_quickview_price">' + price_get(count) + '</div>')) + '\
                              <div class="launchtip_quickview_atc">' + quickview_atc_html + '</div>\
                              <div class="launchtip_quickview_description">' + (prod["description"] || '') + '</div>\
                              </div>\
                              </div>\
                              ...
                              <div id="checkbox__'+prod["handle"]+'" class="onpage_item">\
                                ...
                                <div class="check_details">\
                                  ...
                                  <p class="title" style="'+title_font_css+'">\
                                    '+title_anchor_open+' title="'+title_get(count)+'" style="'+titlestyletoapply+'">'+title_get(count)+'</a>\
                                  </p>\
                                  <div>\
              ...
              function get_product(upsell_data, sellup_id,each_variant_class_data){
                  ...
                  if(value1.length == 1 && key1 != ""){
                    ...
                    if (value1.length > 0) {
                      ...
                      $.each(value1, function(key, value) {
                        ...
                        var f_size = setting.title_font_size != '' ? ltSanitizeSize(setting.title_font_size) + 'px;' : '20px;'
                        ...
                        if(setting.title_status == false){
                          ...
                          if (value.offer_title_status == false){
                            if (value.offer_customize_status && value.offer_title_text != "") {
                              var section_offer_title = '<div class="dynamic_title_upsell_section test1 '+ class_data_variable_variant +' " style="font-size:' + f_size + 'font-weight: 600;"><span>' + ltEscapeHtml(value.offer_title_text) + '</span></div>';
                            } else {
                              var section_offer_title = '<div class="dynamic_title_upsell_section" style="font-size:' + f_size + 'font-weight: 600;"><span>' + ltEscapeHtml(setting.title_text) + '</span></div>';
                            }
                          }else{
                            var section_offer_title = '';
                          }
                      ...
                      if(setting.title_status == false){
                        ...
                        if (key1 != "") {
                          ...
                          if (launchtip_frequent_bounght == "false"|| $('.dynamic_title_upsell_section.test').length === 0){
                           stack_onpage_title = '<div class="dynamic_title_upsell_section test" style="font-size:' + f_size + 'font-weight: 600;"><span>' + ltEscapeHtml(setting.title_text) + '</span></div>';
                ...
                if (setting.up_margin_top != '') {
                  wrapperstyletoapply += 'margin-top:' + ltSanitizeSize(setting.up_margin_top) + 'px;'
                }
                if (setting.up_margin_bottom != '') {
                  wrapperstyletoapply += 'margin-bottom:' + ltSanitizeSize(setting.up_margin_bottom) + 'px;';
                }
                ...
                if (cfinalhtml != '') {
                  if ($("#launchtip_upsell_select_wrapper").length) {
                    ...
                    var wrapper_styleElement = document.createElement('style');
                      wrapper_styleElement.type = 'text/css';

                      wrapper_styleElement.textContent =
                        '#launchtip_upsell_select_wrapper {' +
                        wrapperstyletoapply +
                        '}';

                      document.head.appendChild(wrapper_styleElement);
              ...
              function get_action_product(upsell_data, sellup_id) {
                ...
                if (product_list != '') {
                  ...
                  if (setting.title_status == true) {
                    var f_size = setting.title_font_size != '' ? ltSanitizeSize(setting.title_font_size) + 'px;' : '20px;'
                    var title_html_sellup = '<div class="dynamic_title_upsell" style="font-size:' + f_size + '"><span>' + ltEscapeHtml(setting.title_text) + '</span></div>';
                  } else {
                  ...
                  $.each(upsell_action_data, function(key, value) {
                    ...
                    var offer_data = "";
                    var popupfontcolor = ltSanitizeColor(setting['popup_font_color']);
                    if (popupfontcolor == '') {
                      popupfontcolor = '#000';
                    }
                    if (upsell_action_data.length > 1) {
                      offer_data = '<span style="color: ' + popupfontcolor + ';font-size: 16px;float: left;">' + ltEscapeHtml(offer_text) + ' ' + offer_number + '/' + product_list.length + '</span>';
                    } else {
                      offer_data = "";
                    }
                    if (upsell_action_data.length == key + 1 || key == product_list.length-1) {
                      var continue_next = '<a href="#" class="continue-button-action" style="color: ' + popupfontcolor + ';font-size: 18px;float: right;margin-right: 2%;text-decoration: underline;">' + ltEscapeHtml(continue_text) + '</a>';
                    } else {
                      var continue_next = '<a href="#" class="next-button-action" data-key="' + key + '" style="color: ' + popupfontcolor + ';font-size: 17px;float: right;margin-right: 2%;text-decoration: underline;">' + ltEscapeHtml(next_text) + '</a></div>'
                    }
                    $("body").append('<div class="actionOfferPopup_' + key + ' upsellmyPopup action-upsell-popup text-left"><input type="hidden" value="' + sellup_id + '" class="upsell_action_sellup_id" ></input><div class="actionpopup_text"><h3>' + ltEscapeHtml(action_title) + '</h3></div><div class="actionpopup_desc" style="padding-top: 1%;font-size: 18px;"><span>' + ltEscapeHtml(action_desc) + '</span></div>' + product_list[key] + offer_data + continue_next);
                  });
              ...
              // });
                ...
                var upsell_data = upsellers
                .filter(item =>
                  item.upseller_lead_products?.some(ulp => ulp.id === String(product_id))
                )
                .map(item => {
                  if (item.offer_customize_status) {

                    const key = `onpage_section_title_${item.id}`;

                    return {
                      ...item,
                      offer_title_text:
                        setting?.translations?.[key]?.[Shopify.locale] ||
                        item.offer_title_text
                    };
                  }
                  return item;
                });
              ...
              var configuration = ({
                ...
                beforeOpen: function(event){
                  var variant_popup_bg_color = (ltSanitizeColor(setting['popup_bg_color']) == '' || ltSanitizeColor(setting['popup_bg_color']) == null) ? '#fff' : ltSanitizeColor(setting['popup_bg_color']);
                  $('.launchtip_upsell_lightbox .sellup_popup_heading').css("color", ltSanitizeColor(setting['popup_font_color']));
                  $('button.featherlight-close').attr("style", "background-color: transparent !important; color: "+ltSanitizeColor(setting['popup_button_color'])+";");
                  $('.launchtip_upsell_lightbox select, .launchtip_upsell_lightbox input').attr("style", "background-color: "+ltSanitizeColor(setting['popup_bg_color'])+" !important; color: "+ltSanitizeColor(setting['popup_font_color'])+";");
                  $('.featherlight-content').attr("style", "background-color: "+variant_popup_bg_color+" !important; border-color: "+ltSanitizeColor(setting['popup_border_color'])+" !important; color: "+ltSanitizeColor(setting['popup_font_color'])+";");
                  $('.launchtip_upsell_lightbox select.launchtip_select_wrapper').hide();
                }
              ...
              document.addEventListener('click', function(e) {
                var thumb = e.target.closest('.launchtip_quickview_thumb');
                if (thumb) {
                  var gallery = thumb.closest('.lbimg');
                  gallery.querySelectorAll('.launchtip_quickview_thumb').forEach(function(el) {
                    el.classList.remove('active');
                  });
                  thumb.classList.add('active');
                  gallery.querySelector('.lightboximage').setAttribute('src', thumb.getAttribute('src'));
                }
              });

              $(document).on('click', '.launchtip_quickview_qty_minus, .launchtip_quickview_qty_plus', function(e) {
                e.preventDefault();
                var input = $(this).closest('.launchtip_quickview_qty_wrapper').find('.launchtip_quickview_qty_input');
                var qty = parseInt(input.val(), 10) || 1;
                qty = $(this).hasClass('launchtip_quickview_qty_minus') ? Math.max(1, qty - 1) : qty + 1;
                input.val(qty);
              });
              ...
              $(document).ready(function() {
                ...
                $(document).on("click", ".next-button-action", function(e) {
                  ...
                  setTimeout(function() {
                    $('.featherlight-content').attr("style", "background-color: " + ltSanitizeColor(setting['popup_bg_color']) + " !important; border-color: " + ltSanitizeColor(setting['popup_border_color']) + " !important; color: " + ltSanitizeColor(setting['popup_font_color']) + ";");
                    $('button.featherlight-close').attr("style", "background-color: transparent !important; color: " + ltSanitizeColor(setting['popup_button_color']) + ";");
                  }, 100);
                });
                ...
                $('.action_add_to_cart').click(function() {
                  var url = '/apps/launchtip' + '/output/view.json?shop=' + shop_name + '&product_id=' + product_id + '&upsell_type=action';
                  ...
                  if (upsell_action_data != "" && div_exists_or_not == 1) {
                    ...
                    $('.featherlight-content').attr("style", "background-color: " + ltSanitizeColor(setting['popup_bg_color']) + " !important; border-color: " + ltSanitizeColor(setting['popup_border_color']) + " !important; color: " + ltSanitizeColor(setting['popup_font_color']) + ";");
                    $('.upsellmyPopup .actionpopup_text h3').attr("style", "color: " + ltSanitizeColor(setting['popup_font_color']) + ";");
                    $('button.featherlight-close').attr("style", "background-color: transparent !important; color: " + ltSanitizeColor(setting['popup_button_color']) + ";");
                  }
                });
    ...
    if(window.jQuery){
      ...
      document.head.appendChild(launchtip_script);  
      launchtip_script.type = 'text/javascript';
      launchtip_script.src = "//ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js";
      launchtip_script.onload = function(){
          jQueryCode(); 
          cartdrawer_upsells();  
      };
    }
      ...
      if (cart_template == "cart"){
        ...
        jQueryCode1 = function(){
          ...
          $(document).ready(function(){
            ...
            var sellup_script = document.createElement('script');
              sellup_script.type = 'text/javascript';
              sellup_script.src =
                'https://cdn.jsdelivr.net/npm/featherlight@1.7.14/release/featherlight.min.js';
              sellup_script.integrity =
                'sha384-w2axFeF9Tj4TdpJI51s6IO16THIEjqTxuxTeDLfLY45ZpuoVkl/s9zF0N3mJg0oX';
              sellup_script.crossOrigin = 'anonymous';
              document.head.appendChild(sellup_script);
              ...
              $.ajax({
                ...
                success: function(data) {
                  if (data['items'].length != 0) {
                    ...
                    items_ids = arr;
                    if (items_ids != undefined) {
                      var url = '/apps/launchtip'+'/output/cart_view.json?shop='+shop_name+'&items='+JSON.stringify(items_ids)+ "&shop_locale=" + Shopify.locale;
                    }
                    // --------------------------end-------------------------
                    $.ajax({
                      crossOrigin: true,
                      ...
                      success: function(data) {
                        ...
                        if (data["success"] == true) {
                          ...
                          var button_class='';
                            ...
                            if(data["settings"]["up_price_font_color"] != ''){
                                pricestyletoapply += 'color:'+ltSanitizeColor(data["settings"]["up_price_font_color"])+';';
                                pricestyletoapply += 'font-size:'+ltSanitizeSize(data["settings"]["up_price_font_size"])+'px;'
                            }
                            if(data["settings"]["up_comp_price_font_color"] != ''){
                                comppricestyletoapply += 'color:'+ltSanitizeColor(data["settings"]["up_comp_price_font_color"])+';';
                                ...
                                var producthtml = function(productdetail,count,cart_upsell_no,selected_id,sellup_id,ind_product_id){
                                    ...
                                    if(prod == null){
                                        ...
                                        if((firstvariantcprice) && (firstvariantcprice != '') && (firstvariantcprice > firstvariantprice)){
                                            ...
                                            cp_html = '<span class="compareprice launchtip_price" style=" text-decoration: line-through; justify-content: center;'+comppricestyletoapply+'">'+firstvariantcprice+'</span>'
                                        }else{
                                          cp_html = '';
                                        }
                                        var addcart_text = ltEscapeHtml(data["settings"]["cart_upsell_addcart"]);
                                        ...
                                        if(data["settings"]["up_button_bg_color"] != ''){
                                          btnstyletoapply += 'background-color:'+ltSanitizeColor(data["settings"]["up_button_bg_color"])+';';
                                        }
                                        if(data["settings"]["up_button_font_color"] != ''){
                                          btnstyletoapply += 'color:'+ltSanitizeColor(data["settings"]["up_button_font_color"])+';';
                                        }
                                        ...
                                        if (vcount == 1 && var_in_stock == false) {
                                          ...
                                          return '<div id="product-' + prod["handle"] + '" class="item productbox upsellproductcount' + count + '">' + 
                                              product_image_container + 
                                              '<div class="details">\
                                            <a href="' + window.Shopify.routes.root + 'products/' + prod["handle"] + '" style="text-decoration: none;color:' + ltSanitizeColor(data["settings"]["up_title_font_color"]) + '">\
